...
int func_def_nargs(void *ptr) {
  Quad *ast = ptr;
  assert(ast != NULL);
  return ast->tableentry->argcnt;
}

/*
 * ptr: pointer to an AST for a function definition, n is an integer.
 * If n > 0 and n <= no. of arguments for the function, then func_def_argname()
 * returns a pointer to the name (a string) of the nth formal parameter for that
 * function; the first formal parameter corresponds to n == 1.
 *
 * If the value of n is outside these parameters, the behavior of this function
 * is undefined.
 */
char *func_def_argname(void *ptr, int n) {
  ...
  int argcnt = ast->tableentry->argcnt;

  if (n > 0 && n <= argcnt) {
    int i = 1;
    Quad *curarg = ast->child0;
